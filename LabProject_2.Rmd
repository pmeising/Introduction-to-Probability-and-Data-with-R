...
delayed for 5 minutes o more to be "delayed".
...
10. Suppose you define a flight to be \"on time\" if it gets to the
    ...
    called arr_type with levels "on time" and "delayed" based on this
    definition. Also mutate to create a new variable called dep_type
    with levels "on time" and "delayed" depending on the flight was
    delayed for fewer than 5 minutes or 5 minutes or more, respectively.
    In other words, if arr_delay is 0 minutes or fewer, arr_type is "on
    time". If dep_delay is less than 5 minutes, dep_type is "on time".

    Then, determine the on time arrival percentage based on whether the
    flight departed on time or not. What fraction of flights that were
    "delayed" departing arrive "on time"? (Enter the answer in decimal
    point, like 0.xx)
...
nycflights %>%
  ...
  summarize(on_time = sum(arr_type == "on time") / n())
  print()
